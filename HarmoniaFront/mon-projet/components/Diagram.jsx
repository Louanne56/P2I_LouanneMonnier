...
const Diagram = () => {
  ...
  const playChordSound = async (accordNom, audio, audio2) => {
    ...
    const audioUrl = getResourceUrl(`/audio/${audioFileName}.mp3`);
          ...
          return diagrams.map((diagram, index) => (
            ...
            >
              <View style={styles.imageContainer}>
                <TouchableOpacity 
                  style={styles.volumeIcon}
                  onPress={() => playChordSound(accord.nom, accord.audio, accord.audio2)}
                >
                  <Icon name="volume-up" size={14} color="#3498db" />
                </TouchableOpacity>
  
                {/* Affichage de l'image du diagramme avec URL dynamique */}
                <Image
